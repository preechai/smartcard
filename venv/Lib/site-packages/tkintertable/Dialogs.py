...
class RecordViewDialog(Dialog):
    ...
    def body(self, main):
        ...
        Label(main, text='Rec Name:').grid(row=0,column=0,padx=2,pady=2,sticky='news')
        Entry(main, textvariable=self.fieldvars['Name']).grid(row=0,column=1,padx=2,pady=2,sticky='news')
        i=1
        for col in cols:
            ...
            self.fieldnames[col] = Label(main, text=col).grid(row=i,column=0,padx=2,pady=2,sticky='news')
            ent = Entry(main, textvariable=self.fieldvars[col])
...
class MultipleValDialog(simpledialog.Dialog):
    ...
    def body(self, main):
        ...
        for i in range(len(self.labels)):
            Label(main, text=self.labels[i]).grid(row=r, column=0,sticky='news')
            if self.types[i] == 'int':
            ...
            if self.types[i] == 'list':
                button=Menubutton(main, textvariable=self.vrs[i])
                ...
                self.entries.append(Entry(main, textvariable=self.vrs[i], show=s))
